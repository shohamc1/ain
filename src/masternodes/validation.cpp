...
static void ProcessRewardEvents(const CBlockIndex *pindex, CCustomCSView &cache, const CChainParams &chainparams) {
    ...
    if (pindex->nHeight >= chainparams.GetConsensus().EunosHeight) {
        const auto &incentivePair =
            chainparams.GetConsensus().newNonUTXOSubsidies.find(CommunityAccountType::IncentiveFunding);
...
std::vector<CAuctionBatch> CollectAuctionBatches(const CCollateralLoans &collLoan,
            ...
            for (uint32_t i = 0; i < data.batchCount; i++) {
                ...
                if (auto bid = view.GetAuctionBid({vaultId, i})) {
                    ...
                    if (amountToFill > 0) {
                        ...
                        SwapToDFIorDUSD(
                            view, bidTokenAmount.nTokenId, amountToFill, tmpAddress, tmpAddress, pindex->nHeight);
                ...
                for (const auto mnId : voters) {
                    ...
                    if (mn->rewardAddressType != 0) {
                        scriptPubKey = GetScriptForDestination(
                            mn->rewardAddressType == PKHashType ? CTxDestination(PKHash(mn->rewardAddress))
                                                                : CTxDestination(WitnessV0KeyHash(mn->rewardAddress)));
                    } else {
                        scriptPubKey = GetScriptForDestination(
                            mn->ownerType == PKHashType ? CTxDestination(PKHash(mn->ownerAuthAddress))
...
static void ProcessMasternodeUpdates(const CBlockIndex *pindex,
    ...
    cache.ForEachNewCollateral([&](const uint256 &key, const MNNewOwnerHeightValue &value) {
        if (value.blockHeight == static_cast<uint32_t>(pindex->nHeight)) {
            ...
            const CKeyID keyId =
                dest.index() == PKHashType ? CKeyID(std::get<PKHash>(dest)) : CKeyID(std::get<WitnessV0KeyHash>(dest));
