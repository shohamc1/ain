...
static std::optional<CTxIn> GetAuthInputOnly(CWalletCoinsUnlocker &pwallet, const CTxDestination &auth) {
...
static CTransactionRef CreateAuthTx(CWalletCoinsUnlocker &pwallet, const std::set<CScript> &auths, int32_t txVersion) {
                ...
                for (const auto &[key, value] : attrMap) {
                    if (const auto attrV0 = std::get_if<CDataStructureV0>(&key)) {
                        if (attrV0->type == AttributeTypes::Consortium &&
                            (attrV0->typeId == 0 || pcustomcsview->GetLoanTokenByID({attrV0->typeId}))) {
